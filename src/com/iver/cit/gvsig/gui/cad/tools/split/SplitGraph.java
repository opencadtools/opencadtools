...
class SplitGraph extends PlanarGraph {
     ...
     *             if no enclosing DirectedEdge is found for <code>edge</code>
	...
	if (edgeEnd == null) {
	    throw new IllegalArgumentException(
		    "No enclosing edge end found for " + edge);
	}
	remove(edgeEnd);
    }
     ...
     * @return
     */
    private LineString removeInteriorEndPoints(Polygon polygon,
	    LineString splitter) {
	...
	if (useFrom == useTo) {
	    throw new IllegalArgumentException(
		    "Line lies completely inside polygon");
	}
	...
	LineString[] interiorRings = new LineString[polygon
		.getNumInteriorRing()];
	...
	Geometry interiorRingCollection = factory
		.createMultiLineString(interiorRings);
	...
	// the right
	addEdges(nodedShell, Location.BOUNDARY, Location.EXTERIOR,
		Location.INTERIOR);
	// hole segments oriented CCW means interior at the left, exterior at
	// the right
	addEdges(nodedHoles, Location.BOUNDARY, Location.INTERIOR,
		Location.EXTERIOR);
	// splitter intersection segments have interior location at both left
	// and right
	addEdges(intersectingLineStrings, Location.BOUNDARY, Location.INTERIOR,
		Location.INTERIOR);
    }

    private void addEdges(Geometry linearGeom, int onLoc, int leftLoc,
	    int rightLoc) {
