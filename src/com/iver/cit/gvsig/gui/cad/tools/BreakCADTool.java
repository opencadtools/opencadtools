...
public class BreakCADTool extends DefaultCADTool {
     ...
     * gc=(FGeometryCollection
     * )((DefaultFeature)rowEdited.getLinkedRow()).getGeometry(); IGeometry[]
     * geoms=gc.getGeometries(); for (int i = 0;i<geoms.length;i++) {
    ...
    private void breakGeometry(DefaultRowEdited dre)
	    throws ReadDriverException, ExpansionFileReadException {
	breakGeom(dre);
    }

    private void breakGeom(DefaultRowEdited dre) throws ReadDriverException,
	    ExpansionFileReadException {
	GeneralPathX newGp1 = new GeneralPathX();
	GeneralPathX newGp2 = new GeneralPathX();
	IGeometry geomAux = ((DefaultFeature) rowEdited.getLinkedRow())
		.getGeometry();
	...
	if (gpx.isClosed()) {
	    ...
	    newGp2.append(newGp1.getPathIterator(null, FConverter.FLATNESS),
		    true);
	    IGeometry geom1 = ShapeFactory.createPolyline2D(newGp2);

	    DefaultFeature dfLine1 = (DefaultFeature) dre.getLinkedRow()
		    .cloneRow();
	    dfLine1.setGeometry(geom1);
	    int indexLine1 = addGeometry(geom1, dfLine1.getAttributes());
	    ...
	    DefaultFeature dfLine2 = (DefaultFeature) dre.getLinkedRow()
		    .cloneRow();
	    dfLine2.setGeometry(geom2);
	    int indexLine2 = addGeometry(geom2, dfLine2.getAttributes());
	...
	if (selectedRows.size() == 1) {
	    if (firstPoint != null) {
		IGeometry g1 = ShapeFactory.createCircle(firstPoint,
			vp.toMapDistance(3));
		IGeometry g2 = ShapeFactory.createCircle(firstPoint,
			vp.toMapDistance(5));
	    ...
	    // .getLinkedRow()
	    // .cloneRow();
	    IGeometry geom = ((DefaultFeature) rowEdited.getLinkedRow())
