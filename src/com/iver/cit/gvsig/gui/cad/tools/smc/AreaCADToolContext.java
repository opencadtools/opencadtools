 ...
 */
public final class AreaCADToolContext extends statemap.FSMContext {
    // ---------------------------------------------------------------
    // Member methods.
    //
    ...
    public AreaCADToolState getState() throws statemap.StateUndefinedException {
	if (_state == null) {
    ...
    public static abstract class AreaCADToolState extends statemap.State {
	// -----------------------------------------------------------
	// Member methods.
	//
	...
	protected void addPoint(AreaCADToolContext context, double pointX,
	...
	protected void removePoint(AreaCADToolContext context,
	...
	static {
	    ...
	    SecondPoint = new Area_Default.Area_SecondPoint("Area.SecondPoint",
		    1);
	    ThirdPoint = new Area_Default.Area_ThirdPoint("Area.ThirdPoint", 2);
	    NextPoint = new Area_Default.Area_NextPoint("Area.NextPoint", 3);
	    HoleOrNextPolygon = new Area_Default.Area_HoleOrNextPolygon(
		    "Area.HoleOrNextPolygon", 5);
	    Default = new Area_Default("Area.Default", -1);
	}

    }

    protected static class Area_Default extends AreaCADToolState {
	// -----------------------------------------------------------
	// Member methods.
	//
	...
	protected void addOption(AreaCADToolContext context, String s) {
		...
		boolean loopbackFlag = context.getState().getName()
			.equals(Area.FirstPoint.getName());
	...
	private static final class Area_FirstPoint extends Area_Default {
	    // -------------------------------------------------------
	    // Member methods.
	    //
		...
		if (ctxt.pointInsidePolygon(pointX, pointY)) {
			...
			ctxt.setQuestion(PluginServices.getText(this,
				"insert_first_point"));
			ctxt.throwInvalidGeometryException(PluginServices
				.getText(this, "multipolygon_forbidden"));
		    } finally {
			context.setState(endState);
		    }
		}
	...
	private static final class Area_SecondPoint extends Area_Default {
	    // -------------------------------------------------------
	    // Member methods.
	    //
	...
	private static final class Area_ThirdPoint extends Area_Default {
	    // -------------------------------------------------------
	    // Member methods.
	    //
		...
		try {
		    ...
		    if (deleteButton3) {
			ctxt.setQuestion(PluginServices.getText(this,
				"insert_next_point_or_hole_del"));
		    } else {
			ctxt.setQuestion(PluginServices.getText(this,
				"insert_next_point_or_hole"));
		    }
	...
	private static final class Area_NextPoint extends Area_Default {
	    // -------------------------------------------------------
	    // Member methods.
	    //
		...
		if (s.equals("espacio")
			|| s.equals(PluginServices.getText(this, "terminate"))) // NACHOV
										// &&
										// ctxt.hasNextGeometry()
										// &&
										// ctxt.checksOnInsertion(ctxt.getCurrentGeom()))
		{
			...
			ctxt.setQuestion(PluginServices.getText(this,
				"insert_first_point"));
			ctxt.fireEndGeometry(AreaCADTool.AREA_ACTION_COMMAND);
		    } finally {
			context.setState(Area.FirstPoint);
			(context.getState()).Entry(context);
			ctxt.clear();
		...
		// else if (s.equals("espacio")) //NACHOV &&
		// (!ctxt.hasNextGeometry()) &&
		// ctxt.checksOnInsertion(ctxt.getCurrentGeom()))
		// {
		//
	...
	private static final class Area_HoleOrNextPolygon extends Area_Default {
	    // -------------------------------------------------------
	    // Member methods.
	    //
