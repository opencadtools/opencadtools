...
public class MoveCADTool extends DefaultCADTool {
	...
	if (selectedRow.size() == 0
	    ...
	    ((SelectionCADTool) CADExtension.getCADTool()).setNextTool("_move");
	}
    }
	...
	if (status.equals("Move.FirstPointToMove")) {
		...
		for (int i = 0; i < selectedRow.size(); i++) {
		    ...
		    selectedRowAux.add(new DefaultRowEdited(feat,
	...
	if (status.equals("Move.SecondPointToMove")) {
	     ...
	     * edRow.getLinkedRow(); IGeometry ig =
	     * feat.getGeometry().cloneGeometry(); if (ig == null) continue; //
	     * Movemos la geometría UtilFunctions.moveGeom(ig, x -
	     * firstPoint.getX(), y - firstPoint.getY()); ig.draw((Graphics2D)
	     * g, getCadToolAdapter().getMapControl().getViewPort(),
