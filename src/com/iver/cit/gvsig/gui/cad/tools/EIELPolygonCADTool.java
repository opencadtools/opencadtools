...
import com.iver.cit.gvsig.gui.cad.tools.smc.EIELPolygonCADToolContext;
 ...
 */
public class EIELPolygonCADTool extends InsertionCADTool {

	private EIELPolygonCADToolContext _fsm;

	/**
	 * Contiene los puntos del polígono digitalizado actualmente (puede ser el anillo
	 * exterior o alguno de los huecos)
	 * */
	...
	public void init() {
		...
		numShapes = 0;
		isHole = false;
		_fsm = new EIELPolygonCADToolContext(this);
...
//		los snappers
	...
	public void addPoint(double x, double y,InputEvent event) {
		AreaCADToolState actualState = (AreaCADToolState) _fsm.getPreviousState();
		String status = actualState.getName();
		//NACHOV
//		if ( status.equals("Area.FirstPoint")){
//			//esto es para que no haya problemas con las entidades compuestas (como carreteras)
...
//		}
	...
	public void removePoint(InputEvent event) {
		...
		if (( status.equals("Area.FirstPoint")) || (status.equals("Area.SecondPoint"))){
			if(numShapes == 0){
				cancel();
...
//			prueba para actualizar el ultimo punto pulsado
		...
		}else if((status.equals("Area.ThirdPoint"))|| (status.equals("Area.NextPoint"))){
//			prueba para actualizar el ultimo punto pulsado
...
//			los snappers
	 ...
	 * Borra la última fila añadida al VectorialEditableAdapter.
	 * Si se estaba digitalizando un hueco sólo se elimina el último
	 * hueco. Si sólo estaba digitalizado el anillo exterior se elimina
	 * la fila entera.
	 * */
...
//					ademas pintamos el punto para que se note el snapin
					if(i<listaPuntos.size()-1){
						Point2D actual = null;
						actual = CADExtension.getEditionManager().getMapControl().getViewPort().fromMapPoint(punto);
						int sizePixels = 12;
