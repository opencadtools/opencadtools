...
public final class PolylineCADToolContext extends statemap.FSMContext {
    // ---------------------------------------------------------------
    // Member methods.
    //
    ...
    public static abstract class PolylineCADToolState extends statemap.State {
	// -----------------------------------------------------------
	// Member methods.
	//
	...
	protected void addPoint(PolylineCADToolContext context, double pointX,
	...
	protected void endPoint(PolylineCADToolContext context, double pointX,
		double pointY, InputEvent event) {
	    Default(context);
	}
	...
	private static final class Polyline_FirstPoint extends Polyline_Default {
	    ...
	    @Override
	    protected void addPoint(PolylineCADToolContext context,
		...
		if (ctxt.isPolygonLayer()) {
			...
			ctxt.setQuestion(PluginServices.getText(this,
				...
				+ PluginServices.getText(this,
					"PolylineCADTool.end") + "]");
			ctxt.setDescription(new String[] { "inter_arc",
				"close_polyline", "terminate", "cancel" });
			ctxt.addPoint(pointX, pointY, event);
		    } finally {
			context.setState(Polyline.NextPointOrArcOrClose);
			(context.getState()).Entry(context);
	...
	private static final class Polyline_NextPointOrArcOrClose extends
		...
		if (ctxt.isPolygonLayer()
			&& (s.equalsIgnoreCase(PluginServices.getText(this,
				"PolylineCADTool.arc")) || s
				.equals(PluginServices.getText(this,
					"inter_arc")))) {
			...
			ctxt.setQuestion(PluginServices.getText(this,
				...
				+ PluginServices.getText(this,
					"PolylineCADTool.end") + "]");
			ctxt.setDescription(new String[] { "inter_line",
				"terminate", "cancel" });
			ctxt.addOption(s);
		    } finally {
			...
			"PolylineCADTool.end")) || s.equals(PluginServices
			.getText(this, "terminate")))
			&& ctxt.getLinesCount() != 0) {
