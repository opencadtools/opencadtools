...
public class SplineCADTool extends DefaultCADTool {
    private static Logger logger = Logger.getLogger(SplineCADTool.class
	    .getName());
	...
	IGeometry newGeom = ShapeFactory.createSpline2D((Point2D[]) list
	...
	close = true;
	Point2D endPoint = new Point2D.Double(((Point2D) list.get(0)).getX(),
	...
	SplineCADToolState actualState = (SplineCADToolState) _fsm
		.getPreviousState();
	String status = actualState.getName();
	if (status.equals("Spline.NextPoint")
		|| status.equals("Spline.FirstPoint")) {
	    list.add(new Point2D.Double(x, y));
	    IGeometry spline = ShapeFactory.createSpline2D((Point2D[]) list
		    .toArray(new Point2D[0]));
	...
	if (status.equals("Spline.NextPoint")
	    ...
	    IGeometry spline = ShapeFactory.createSpline2D((Point2D[]) points
		    .toArray(new Point2D[0]).clone());
