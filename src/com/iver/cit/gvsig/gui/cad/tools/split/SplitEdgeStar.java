...
class SplitEdgeStar extends DirectedEdgeStar {
    ...
    /**
     * Finds the first edge to the passed in in the
     * <code>searchDrirection</code> direction.
     * 
     ...
     *         are no edges in the prefferred direction.
     */
    public DirectedEdge findClosestEdgeInDirection(DirectedEdge edge,
	...
	if (CGAlgorithms.CLOCKWISE != searchDirection
	    ...
	    throw new IllegalArgumentException(
		    "Allowed values for for searchDirection "
	...
	if (super.findIndex(edge) == -1) {
	    ...
	    if (super.findIndex(edge) == -1) {
		throw new IllegalArgumentException(
			"Edge does not belongs to this edgestar");
	    }
	}
	final int degree = getDegree();
	if (degree < 2) {
	    throw new IllegalStateException(
		    "there must be at least two edges in the edge star");
	}
	final Coordinate nodeCoord = getCoordinate();
	...
	for (Iterator it = outgoingEdges.iterator(); it.hasNext();) {
	    ...
	    double angle = computeAngleInDirection(tip1, tail, tip2,
		    searchDirection);
     ...
     */
    public double computeAngleInDirection(Coordinate tip1, Coordinate tail,
	    Coordinate tip2, int direction) {
	final int orientation = CGAlgorithms.computeOrientation(tail, tip1,
		tip2);
