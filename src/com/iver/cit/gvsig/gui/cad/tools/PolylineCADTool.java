...
public class PolylineCADTool extends DefaultCADTool {
	...
	PolylineCADToolState actualState = (PolylineCADToolState) _fsm
		.getPreviousState();
	...
	if (status.equals("Polyline.NextPointOrArcOrClose")
		...
		if (!(list.size() > 0)
		    ...
		    Point2D centerp = UtilFunctions.getPoint(interp, mediop,
			    radio);
		    antCenter = centerp;
		    IGeometry geom = ShapeFactory.createArc(lastp, centerp,
			    point);
		    list.add(geom);
		    addTemporalCache(geom);
		}
	...
	try {
	    if (getVLE().getVEA().getShapeType() == FShape.POLYGON
		    && !list.isEmpty()) {
		Handler handler1 = ((IGeometry) list.get(0))
			.getHandlers(IGeometry.SELECTHANDLER)[0];
	...
	/*
	 * PolylineCADToolState actualState = (PolylineCADToolState)
	 * _fsm.getPreviousState(); String status = actualState.getName();
	 * 
	 * if (status.equals("Polyline.NextPointOrArcOrClose")) { if
	 * (s.equals("A") || s.equals("a") ||
	 * s.equals(PluginServices.getText(this,"inter_arc"))) { //Arco } else
	 * if (s.equals("C") || s.equals("c")) { GeneralPathX elShape = new
	 * GeneralPathX(GeneralPathX.WIND_EVEN_ODD, 2);
	 * elShape.moveTo(antPoint.getX(), antPoint.getY());
	 * elShape.lineTo(firstPoint.getX(), firstPoint.getY());
	 * list.add(ShapeFactory.createPolyline2D(elShape)); //closeGeometry();
	 * } } else if (status.equals("Polyline.NextPointOrLineOrClose")) { if
	 * (s.equals("N") || s.equals("n") ||
	 * s.equals(PluginServices.getText(this,"inter_line"))) { //Línea } else
	 * if (s.equals("C") || s.equals("c")) { GeneralPathX elShape = new
	 * GeneralPathX(GeneralPathX.WIND_EVEN_ODD, 2);
	 * elShape.moveTo(antPoint.getX(), antPoint.getY());
	 * elShape.lineTo(firstPoint.getX(), firstPoint.getY());
