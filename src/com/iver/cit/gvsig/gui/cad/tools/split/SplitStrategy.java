 ...
 * 
 * We have adapted it slightly for the gvsig project (exchange the I18 system,
 * etc)
 * 
...
public class SplitStrategy {
	...
	knownStrategies.put(MultiLineString.class,
		MultiLineStringSplitter.class);
    ...
    /**
     * @param splitee
     * @return a <code>Geometry</code> containing all the splitted parts as
     *         aggregates. Use {@link Geometry#getGeometryN(int)
     *         getGeometryN(int)} to get each part.
     * @throws NullPointerException
     *             if geom is null
     * @throws IllegalArgumentException
     *             if geom is not of an acceptable geometry type to be splitted
     *             (i.e. not a linestring, multilinestring, polygon or
     *             multipolygon)
     */
	...
	if (!strategies.containsKey(spliteeClass)) {
	    throw new IllegalArgumentException(
		    "La geometria especificada no soporta la operacion 'split'");
	}
	...
	try {
	    ...
	    throw (RuntimeException) new RuntimeException(
		    "Illegal access exception for " + splitOpClass.getName())
		    .initCause(e);
	}
	return splitOp;
    }
	...
	/**
	 * No-op default constructor required to reflectively instantiate the
	 * class
	 */
	...
	/**
	 * @param splitee
	 *            the {@link LineString} to be splitted
	 */
	@Override
	public Geometry split(Geometry splitee) {
     ...
     * @since 1.1.0
     */
    private static abstract class AbstractGeometryCollectionSplitter implements
	    SpecificSplitOp {

	private SpecificSplitOp singlePartSplitter;

	private AbstractGeometryCollectionSplitter(
		SpecificSplitOp singlePartSplitter) {
	...
	public final Geometry split(final Geometry splitee) {
		...
		for (int splittedPartN = 0; splittedPartN < splittedPartsCount; splittedPartN++) {
		    Geometry simpleSplittedPart = splittedPart
			    .getGeometryN(splittedPartN);
		    splittedParts.add(simpleSplittedPart);
		}
	    }

	    GeometryFactory gf = splitee.getFactory();
	    GeometryCollection splittedCollection = buildFromParts(gf,
		    splittedParts);
	    return splittedCollection;
	}

	protected abstract GeometryCollection buildFromParts(
		GeometryFactory gf, List parts);
     ...
     * @since 1.1.0
     */
    private static class MultiLineStringSplitter extends
	    AbstractGeometryCollectionSplitter {
	...
	@Override
	protected GeometryCollection buildFromParts(GeometryFactory gf,
		List parts) {
	    LineString[] lines = (LineString[]) parts
		    .toArray(new LineString[parts.size()]);
     ...
     * @since 1.1.0
     */
    private static class MultiPolygonSplitter extends
	    AbstractGeometryCollectionSplitter {
	...
	protected GeometryCollection buildFromParts(GeometryFactory gf,
		List parts) {
	    Polygon[] polygons = (Polygon[]) parts.toArray(new Polygon[parts
		    .size()]);
     ...
     * Estrategia:
     * <ul>
     * <li>Construir un grafo con todos los edges y nodes de la intersección
     * del polígono con la línea
     * <li>Ponderar los nodos según la cantidad de edges incidentes. Nodos son
     * solo las intersecciones del boundary del poligono con el linestring y el
     * punto inicial de cada parte del boundary.
     * <li>Ponderar los edges según son shared (todos los del linestring) o non
     * shared (todos los del boundary del poligono). Almacenar la lista de
     * coordenadas del edge en el edge.
     * <li>Comenzar a recorrer el grafo por un nodo cualquiera, empezando por su
     * primer edge
     * <li>Recorrer siempre hacia el nodo siguiente, seleccionando el edge cuyo
     * primer segmento de su linestring presenta un menor angulo a la izquiera
     * (CCW) con el ultimo segmento del linestring del edge en curso.
     ...
     * <li>Marcar los edges restantes que tengan algun nodo con ponderación < 3
     * como non-shared
	...
	private Geometry splitPolygon(final Polygon geom) {
	    ...
	    List<Polygon> resultingPolygons = buildSimplePolygons(allRings,
		    unsplittedHoles, gf);
		...
		Polygon[] array = resultingPolygons
			.toArray(new Polygon[resultingPolygons.size()]);
	...
	/**
	 * Finds out and removes from the graph the edges that were originally
	 * holes in the polygon and were not splitted by the splitting line.
	 * 
	 * @param graph
	 * @param gf
	 * @return
	...
	private List<LinearRing> findUnsplittedHoles(SplitGraph graph,
		GeometryFactory gf) {
	    final List<LinearRing> unsplittedHoles = new ArrayList<LinearRing>(
		    2);
	...
	private List<Polygon> buildSimplePolygons(
		List<List<SplitEdge>> allRings,
		...
		List<LinearRing> thisPolyHoles = new ArrayList<LinearRing>(
			unsplittedHoles.size());
		...
		if (numHoles > 0) {
		    LinearRing[] holes = thisPolyHoles
			    .toArray(new LinearRing[numHoles]);
		    LinearRing shell = gf.createLinearRing(poly
	...
	private Polygon buildPolygon(List<SplitEdge> edgeList,
		GeometryFactory gf) {
	...
	/**
	 * Removes from <code>graph</code> the edges in <code>ring</code> that
	 * are no more needed
	 * 
	 * @param graph
	 * @param ring
	...
	/**
	 * Returns the first edge found that belongs to the shell (not an
	 * interior edge, not one of a hole boundary)
	 * <p>
	 * This method relies on shell edges being labeled
	 * {@link Location#EXTERIOR exterior} to the left and
	 * {@link Location#INTERIOR interior} to the right.
	 * </p>
	 * 
	 * @param graph
	 * @return the first shell edge found, or <code>null</code> if there are
	 *         no more shell edges in <code>graph</code>
	 */
