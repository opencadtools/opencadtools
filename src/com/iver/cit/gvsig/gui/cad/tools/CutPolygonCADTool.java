...
public class CutPolygonCADTool extends DefaultCADTool {
    ...
    private IGeometry selectedGeom; // [LBD] Storing the geometry which contains
				    // the first point
	...
	CutPolygonCADToolState actualState = (CutPolygonCADToolState) _fsm
		.getPreviousState();
	...
	if (selectedRows.size() > 0) {
		...
		case PathIterator.SEG_MOVETO:
		    ...
		    numberMultiActual++;
		    // Initializing each time in order to storing only the
		    // geometry of the multi geometry
		    // where the point is
		    //
		    if (multiSelected != 0) {
			terminated = true;
	    ...
	    if (!isInside) {
		// The point pressed will be inside the polygon but wont be
		// inside the line
     ...
     * intersection point was established
     * 
     * @param x
     * @param y
     * @return
	...
	System.out.println("------>>>>>> llamada a secondPointInsideFeature "
		+ x + ", " + y);
	...
	IGeometry ig = null;
	if (selectedRows.size() > 0) {
	    // The point presed is inside the feature of the selected layer so
	    // we must check if is also
	...
	// TODO [NachoV] We must pay attention if there are holes. Actually does
	// not work!!!!
	...
	while (!theIterator.isDone()) {
	    ...
	    switch (theType) {
	    case PathIterator.SEG_MOVETO:
		System.out.println(numberMultiActual
