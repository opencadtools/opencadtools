...
public class ChooseGeometryType extends JWizardPanel {
	...
	if (jPanelGeometryTypeOptions == null) {
	    ...
	    jPanelGeometryTypeOptions
		    .setBorder(javax.swing.BorderFactory.createTitledBorder(
			    null,
			    PluginServices.getText(this, "Geometry_types"),
    ...
    /**
     * En función de qué tipo de driver sea, habilitamos o deshabilitamos las
     * opciones. Por ejemplo, si es de tipo shp, deshabilitamos multi_type
     * 
     * @param writer
     */
	...
	getJRadioButtonPoint()
		.setEnabled(writer.canWriteGeometry(FShape.POINT));
	getJRadioButtonMultiPoint().setEnabled(
		writer.canWriteGeometry(FShape.MULTIPOINT));
	getJRadioButtonLine().setEnabled(writer.canWriteGeometry(FShape.LINE));
	getJRadioButtonPolygon().setEnabled(
		writer.canWriteGeometry(FShape.POLYGON));
	getJRadioButtonMulti()
		.setEnabled(writer.canWriteGeometry(FShape.MULTI));
    }
