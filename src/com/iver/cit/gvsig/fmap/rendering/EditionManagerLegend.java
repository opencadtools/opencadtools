...
public class EditionManagerLegend implements EditionLegend {
	...
	if (vectorialLegend instanceof VectorialUniqueValueLegend) {
	    value = (Value) ((VectorialUniqueValueLegend) vectorialLegend)
		    .getValues()[i];
	} else if (vectorialLegend instanceof VectorialIntervalLegend) {
	    value = (Value) ((VectorialIntervalLegend) vectorialLegend)
		    .getValues()[i];
	} else {
	    value = ValueFactory.createValue(PluginServices.getText(this,
		    "todos_los_valores"));
	}
	return value;
    }
	...
	ISymbol symbol = null;
	symbol = ((IVectorialUniqueValueLegend) vectorialLegend)
		.getSymbolByValue(getValue(i));
	return symbol;
    }
	...
	if (vectorialLegend instanceof VectorialUniqueValueLegend
		|| vectorialLegend instanceof VectorialIntervalLegend) {
	...
	if (!value.equals(previousValue)) {
	    ((IVectorialUniqueValueLegend) vectorialLegend)
		    .delSymbol(previousValue);
	    clave = (Value) value;
	    ((IVectorialUniqueValueLegend) vectorialLegend).addSymbol(value,
		    previousSymbol);
